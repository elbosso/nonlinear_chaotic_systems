...
def solve_sprottm(numberOfTrajectories=10, min_x0=-0.3, max_x0=0.0, anglex=0.0, angley=30.0, max_time=16.0):
    ...
    def sprottm_deriv(x_y_z, t0):
        """Compute the time-derivative of a Sprott system M."""
        x, y, z = x_y_z
        return [-z, -x*x - y, 1.7 + 1.7*x + y]
    ...
    x_t = np.asarray([integrate.odeint(sprottm_deriv, x0i, t)
                      for x0i in x0])
